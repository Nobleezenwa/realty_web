...
const ResetPassword: React.FC = () => {
  ...
  const {email: _email} = location.state? location.state : {email: ""};
  ...
  const submit = async (event: any) => {
      ...
      await request({
        method: 'POST',
        url: config.backend + '/api/user/reset-password',
        formData,
        callback: (res)=> {
          toast(res.data.message);
          navigate('/auth/signin');
          toast("Proceed to login");
        },
  ...
  const resend = async (event: any)=> {
    ...
    await request({
      method: 'POST',
      url: config.backend + '/api/user/forgot-password',
      formData,
              ...
              <div className="mb-4">
                ...
                <div className="relative">
                  <input
                    ...
                    placeholder="XXXXXX"
                    maxLength={6}
                    className="max-w-48 tracking-[5px] rounded-lg border border-stroke bg-transparent py-4 pl-6 pr-10 text-black outline-none focus:border-primary focus-visible:shadow-none dark:border-form-strokedark dark:bg-form-input dark:text-white dark:focus:border-primary"
                  />
                </div>
              </div>
