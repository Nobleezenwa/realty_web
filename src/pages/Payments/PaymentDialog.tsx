...
const PaymentDialog: React.FC<any> = ({closeFn, successFn, failFn, data: _data}) => {
  ...
  const [imagePreview, setImagePreview] = React.useState<any>((typeof data.proof_of_payment !== "undefined")? config.storagePath + '/' + data.proof_of_payment : null);
      ...
      <form className="flex flex-col gap-5.5">
        ...
        <div className="max-w-full flex justify-between gap-4">
          <div className="max-w-[48%]">
            <label className="mb-1 block text-bsslack dark:text-white">
              Select Sale
            </label>
            <SearchBar
              disabled={(mode != 'edit')}
              ...
              className="xmax-w-1/2 rounded-lg border-[1.5px] border-stroke bg-transparent py-3 px-5 text-black outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:text-white dark:focus:border-primary"
            />
          </div>
