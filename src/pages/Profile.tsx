...
const Profile = () => {
  ...
  useEffect(()=> {
    if (profile && profile.profile_photo){ 
      setImagePreview(config.storagePath + '/' + profile.profile_photo)
    } else {
  ...
  const shareRef = async ()=> {
    var shareInfo = {
      title: profile.username + ' - ' + config.appName,
      text: "You are invited to join " + config.appName,
      url: config.frontend + '/auth/signup?ref=' + profile.username,
    };
    try {
      ...
      <div className="overflow-hidden rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
        ...
        <div className="px-4 pb-6 text-center lg:pb-8 xl:pb-11.5">
          <div className="flex justify-center items-center relative z-30 mx-auto -mt-22 h-30 w-full max-w-30 rounded-full bg-white/20 p-1 backdrop-blur sm:h-44 sm:max-w-44 sm:p-3">
              <img src={imagePreview} alt="profile" className="w-30 h-30 rounded-full" />
          </div>
          <div>
            ...
            <p className="font-medium">
              {
                ...
                (profile.is_suspended) ? 'Suspended' :
                    profile.level == constants.accessLevels.ACCESS_LEVEL_NONE ? 'Guest' :
                      profile.level == constants.accessLevels.ACCESS_LEVEL_DOWNLINER ? 'Downliner' :
                        profile.level == constants.accessLevels.ACCESS_LEVEL_UPLINER ? 'Upliner' :
                          profile.level == constants.accessLevels.ACCESS_LEVEL_ADMIN ? 'Admin' : 'Super Admin'
                : ""
              }
            </p>
            ...
            <div className="mt-8 mb-5.5 mx-auto max-w-180">
              ...
              <p className={"mt-4.5"+(profile && profile.bio)? "" : ' italic'}>
            ...
            <div className="mt-8">
              ...
              <div className="flex items-center justify-center gap-3.5">
                {/** Facebook */}
                <Link
                  to={(profile && profile.social_facebook)? profile.social_facebook : '#'}
                ...
                {/** X */}
                <Link
                  to={(profile && profile.social_x)? profile.social_x : '#'}
                  className="hover:text-primary"
                  aria-label="social-icon"
                >
                ...
                {/** LinkedIn */}
                <Link
                  to={(profile && profile.social_linkedin)? profile.social_linkedin : '#'}
                  className="hover:text-primary"
                  aria-label="social-icon"
                >
                  <svg
                ...
                {/** Instagram */}
                <Link
                  to={(profile && profile.social_instagram)? profile.social_instagram : '#'}
                  className="hover:text-primary"
                  aria-label="social-icon"
                >
