...
interface SalesTableProps {
  data: any,
  viewSale: (property_id: number|string)=> void,
  editSale: false|((property_id: number|string)=> void),
}

const SalesTable: React.FC<SalesTableProps> = ({data, viewSale, editSale}) => {
    ...
    <>
        ...
        <div className="max-w-full overflow-x-auto">
          ...
          {data.map((s: any) => (
            <div
              ...
              key={s.id}
            >
              <div onClick={()=>viewSale(s.id)} className="col-span-3 flex items-center">
              ...
              <div className="col-span-1 flex items-center justify-center space-x-3.5">
                <ContextMenu
                    trigger={
                      ...
                      items={[
                        {label: 'View', action: ()=>viewSale(s.id) },
                        ...((editSale)? [{label: 'Edit', action: ()=>editSale(s.id) }] : []),
                        {label: 'Call', action: ()=>window.open("tel:"+s.phone) },
                        {label: 'Email', action: ()=>window.open("mailto:"+s.email) }
                      ]}
                  />
              </div>
            </div>
      ...
      
      {
        ...
        <div className='px-2 pt-24'>
          <p className="text-3xl text-center">No sales to show.</p>
        </div>
      }        
    </>
  );
