    ...
    <>
      <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
        <div className="max-w-full overflow-x-auto">
          <div className="min-w-[500px] grid grid-cols-6 gap-2 border-stroke py-4.5 px-4 dark:border-strokedark sm:grid-cols-8 md:px-6 2xl:px-7.5">
            <div className="col-span-3 flex items-center">
              <p className="font-medium">Commission Info</p>
            </div>
          ...
          {data.map((s: any) => (
            <div
              className="min-w-[500px] grid grid-cols-6 gap-2 border-t border-stroke py-4.5 px-4 dark:border-strokedark sm:grid-cols-8 md:px-6 2xl:px-7.5"
              key={s.id}
            >
              <div onClick={()=>viewCommission(s.id)} className="col-span-3 flex items-center">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
                  <div className="h-12.5 w-15 rounded-md">
                    <img src={config.storagePath + '/' + (s.property? s.property.display_image : '')} alt="Property" />
                  </div>
                  <div>
                    <h5 className="font-medium text-black dark:text-white">
                      {s.title} {s.first_name} {s.last_name} 
                    </h5>
                    <p className="text-sm">Sale ID: {s.id}</p>
                    <p className="text-sm">Property: {(s.property? s.property.name : '')}</p>
