...
const DropdownUser = () => {
  ...
  const load = async ()=> {
    if (!profile) {
      ...
      await request({
        method: 'GET',
        url: config.backend + '/api/user/profile',
  ...
  return (
    <ClickOutside onClick={() => setDropdownOpen(false)} className="relative">
      <Link
        ...
        to="#"
      >
        <span className="text-right lg:block max-w-[100px]">
          <span className="block text-sm font-medium text-black dark:text-white w-full truncate">
            {profile && profile.username}
          </span>
          <span className="block text-xs"
            dangerouslySetInnerHTML={{
                __html: profile?
                  (profile.is_suspended) ? '<span style="color: red;">Suspended</span>' :
                      profile.level == constants.accessLevels.ACCESS_LEVEL_NONE ? 'Guest' :
                        profile.level == constants.accessLevels.ACCESS_LEVEL_DOWNLINER ? 'Downliner' :
                          profile.level == constants.accessLevels.ACCESS_LEVEL_UPLINER ? 'Upliner' :
                            userSession.as != 'admin'? 'Realtor' : 
                              profile.level == constants.accessLevels.ACCESS_LEVEL_ADMIN ? 'Admin' : '<span style="color: green;">Super Admin</span>'
                   : ""
            }}
          ></span>
        </span>
        ...
        <span className="h-12 w-12 rounded-full overflow-hidden">
          <img src={imagePreview} alt="User" className="w-full h-full" />
        </span>
